...
export class TabHistoryModal extends Modal {
  ...
  constructor(
    ...
    private _isPrevCommand: boolean,
  ) {
    super(app);
  ...
  onOpen(): void {
    if (!this._isEnabled) {
      new Notice(
        '"Tab Selector" plugin has incorrect settings. Please review the [For "Go to previous/next tab" command] settings.',
        0,
      );
      this.close();
      return;
  ...
  private generateButtons(contentEl: HTMLElement, leaves: CustomWsLeaf[]): void {
    leaves.forEach((leaf) => {
      contentEl.createDiv('th-leaf-row', (el) => {
        ...
        leafBtnEl.addEventListener(
          'mouseup',
          (ev: MouseEvent) => (ev.preventDefault(), this.switchToFocusedTab(leaf)),
        );
        leafBtnEl.addEventListener(
          'touchend',
          (ev: MouseEvent) => (ev.preventDefault(), this.switchToFocusedTab(leaf)),
        ...
        closeBtnEl.addEventListener(
          'mouseup',
          (ev: MouseEvent) => (ev.stopPropagation(), this.clickCloseLeafButton(leaf, el)),
        );
        closeBtnEl.addEventListener(
          'touchend',
          (ev: MouseEvent) => (ev.stopPropagation(), this.clickCloseLeafButton(leaf, el)),
  ...
  private focusToNextTab(): void {
    if (this._focusPosition === 0) {
      ...
      (
        this._leafButtonMap.get(this._leaves[this._focusPosition - 1].id || '') as HTMLElement
      ).focus();
  ...
  private addMarkOfFocus(): void {
    ...
    const focusLeafEl = this._leafButtonMap.get(
      this._leaves[this._focusPosition]?.id || '',
    ) as HTMLElement;
