...
export class TabShortcutsModal extends Modal {
  ...
  onOpen(): void {
    ...
    this._tabHeaderContainers.forEach((container) =>
      container?.addClass('tsh-header-container-inner'),
    );
  }

  onClose(): void {
    this._windows.forEach(({ window }) =>
      window.removeEventListener('click', this._eventListenerFunc.click),
    );
    ...
    this._tabHeaderContainers.forEach((container) =>
      container?.removeClass('tsh-header-container-inner'),
    );
  ...
  private showShortcutElements(windows: WindowItem[]): void {
    windows.forEach((win) => {
      ...
      this._tabHeaderContainers.push(
        tabContainer?.querySelector('.workspace-tab-header-container-inner'),
      );
      const headers = tabContainer?.querySelectorAll(
