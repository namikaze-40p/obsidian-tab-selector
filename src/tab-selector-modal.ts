...
export class TabSelectorModal extends Modal {
  ...
  private get currentLeaves(): CustomWsLeaf[] {
    return this._leaves.slice(
      0 + this._pagePosition * this._chars.length,
  ...
  constructor(
    ...
    this._leaves = this._leaves.map((leaf) => {
      ...
      const splitPaths = fullPath
        .split('/')
        .map((path) => (path.length > 20 ? `${path.slice(0, 20)}...` : path));
      leaf.path =
        splitPaths.length > 3
          ? `.../${splitPaths.at(-3)}/${splitPaths.at(-2)}/`
          : splitPaths.join('/');
  ...
  private generateDummyButtons(contentEl: HTMLElement, leaves: CustomWsLeaf[]): void {
    const dummyButtonCount =
      this._leaves.length > this._chars.length ? this._chars.length - leaves.length : 0;
  ...
  private reflectOptions(
    leaf: CustomWsLeaf | null,
    el: HTMLDivElement,
    itemBtnEl: HTMLButtonElement,
    itemNameEl: HTMLSpanElement,
  ): void {
    if (
      (this.modalSettings.showAliases && !this.modalSettings.replaceToAliases) ||
      this.modalSettings.showPaths
    ) {
    ...
    switch (key) {
      case UP_KEY:
        if (this._focusPosition === 0) {
          ...
          (
            this._leafButtonMap.get(
              this.currentLeaves[this._focusPosition - 1].id || '',
            ) as HTMLElement
          ).focus();
      ...
      case LEFT_KEY: {
        ...
        if (this._pagePosition === 0) {
          this._pagePosition =
            this._leaves.length % this._chars.length === 0
      ...
      case RIGHT_KEY: {
        ...
        const lastPage =
          this._leaves.length % this._chars.length === 0 ? pageSize - 1 : Math.floor(pageSize);
